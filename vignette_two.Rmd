...
```{r show-marketIds, echo=FALSE}

```

## R script

A script will need to login, find the correct markets, retreive the relevant data, and then save the data.  There are a couple of steps which will be unnecessarily repeated, the login and finding the correct markets.  These can be done once, and then saved, although the script needs to be able to run straight off the bat, so it might look something like the code below
...
A similar `if else` section can be applied for marketIds:

```{r example-marketIds, eval=FALSE}
